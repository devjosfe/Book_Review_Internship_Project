...
async function main() {
  ...
  // Loop through your book data and create records
  for (const data of bookData) {
    const book = await prisma.book.create({
      data: {
        title: data.title, // Corrected: data.title maps to schema.name
        country: data.country,
        coverImg: data.imageLink,
        Language: data.language,
        Author: data.author, // Corrected: data.author maps to schema.
        Year: String(data.year),
        page: data.pages,
        addedBy: data.admin,
        Link: data.link,
        // createdAt will be defaulted by Prisma
        // reviews and readList are relation fields, don't set them directly here unless creating nested data
      },
    });
    console.log(`Created book with id: ${book.id} - ${book.title}`);
  }
